(* A PASCAL - LANGUAGE IMPLEMENTATION OF C-SCAN DISK SCHEDULING ALGORITHM*)
PROGRAM CSCANDiskScheduling(Input, Output);
uses dos;
Const
	MAX  = 199;
 	MIN  = 0;
 	SIZE = 50;
Type 
	Disk = ARRAY[1..SIZE + 2] OF INTEGER;
Var
	n, h, count, i  : integer;
    A, ST, T 	: Disk; 
  	AST		: Real;
...
Procedure CSCAN(a : Disk; st : Disk; var t : Disk; N : Integer; h : integer; var cnt : integer; var ast : real);
Var    
	i, J, L,  R, Temp, Temp1 : integer;
	LE, RE : Disk;
Begin
	 J := 1; L := 1; R := 1;
     for i := 1 to N do
     ...
     Begin
		t[J] := Temp; 
		cnt := cnt + abs(Temp1 - t[J]);
		st[J] := cnt;  Temp1 := t[J]; Temp := MIN; J := J + 1;      
     end;
	 For i := 1 To (L - 1) Do
     Begin
           t[J] := LE[i]; 
...
Begin
  ...
  count := 0;
  writeln('*********** C-SCAN DISK SCHEDULING ALGORITHM IN PASCAL *****************');
  Repeat
  ...
  Writeln('The New(C-SCAN) list is: '); display(T, n+2);
